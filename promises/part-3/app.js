$(function() {
    ...
    // Make a single request to Pokemon API
    $.getJSON(`${baseURL}/pokemon/?limit=1000`).then(data => {
      console.log(data);
    });
  
    // Get all pokemon and log 3 random ones.
    ...
    // Get all pokemon and log the descriptions and species name of the pokemon.
    let names = null;
    $.getJSON(`${baseURL}/pokemon/?limit=1000`)
      .then(data => {
        let randomPokemonUrls = [];
    ...
    // Create UI for creating pokemon and displaying them as cards on the page.
    let $btn = $("button");
